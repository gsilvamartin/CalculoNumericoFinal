function gerarMatrizSeidel() {
    var table = "";
    var matrizFinal = [];
    var numero = parseInt($("#seidel-dimension").val());
    ...
    for (var i = 1; i < matrizFinal.length - 1; i++) {
      if (i === 1) {
        table += "<table id='seidel' style='width:100%' class='table'><tr>";
    ...
    table +=
      "</table><br><button class='btn btn-primary' onclick='calculaseidel()'>Calcular</button>";
  
    $("#seidel-table").html("");
    $("#seidel-table").append(table);
  }
  
  function calculaseidel() {
    var arrayB = [];
    var finalArray = [];
    var epislon = parseInt($("#epislon-seidel").val());
    var table = document.getElementById("seidel").querySelectorAll("#seidel tr");
  ...
  function EfetuaCalculosSeidel(matA, matB, epislon) {
    ...
    $("#resultado-seidel").append(respostaFinal);
    $("#resultado-seidel").show();
  }
